...
inf_left : ∀ (a b : α), a ∈ S → b ∈ S → r (inf a b) a
inf_right : ∀ (a b : α), a ∈ S → b ∈ S → r (inf a b) b
inf_higher : ∀ (a b c : α), a ∈ S → b ∈ S → c ∈ S → r c a → r c b → r c (inf a b)

class has_sup (α : Type u) (r : α → α → Prop) (S : set α) : Type u where
sup : α → α → α
sup_mem : ∀ (a b : α), a ∈ S → b ∈ S → sup a b ∈ S
sup_left : ∀ (a b : α), a ∈ S → b ∈ S → r a (sup a b)
sup_right : ∀ (a b : α), a ∈ S → b ∈ S → r b (sup a b)
sup_lower : ∀ (a b c : α), a ∈ S → b ∈ S → c ∈ S → r a c → r b c → r (sup a b) c
