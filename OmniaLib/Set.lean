...
instance p : Order.partial_order (set α) subset := Order.partial_order.mk


theorem foo (α : Type u) (A B C : set α) (h1 : subset° A B) (h2 : subset° B C) : subset° A C := by
exact @Order.strict_trans (set α) subset p A B C h1 h2
